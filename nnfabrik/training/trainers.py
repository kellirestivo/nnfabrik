...
def verbose_gauss_trainer(model, seed, stop_function='corr_stop',
                       loss_function='PoissonLoss', epoch=0, interval=1, patience=10, max_iter=75,
                       maximize=True, tolerance=0.001, device='cuda', restore_best=True,
                       lr_init=0.005, lr_decay_factor=0.3, min_lr=0.0001, optim_batch_step=True,
                       verbose=True, lr_decay_steps=3, dataloaders=None, gauss_sample=False, **kwargs):
    ...
    def run(model, full_objective, optimizer, scheduler, stop_closure, train_loader,
        ...
        for epoch, val_obj in early_stopping(model, stop_closure,
            ...
            for batch_no, (data_key, data) in tqdm(enumerate(LongCycler(train_loader)),
                ...
                mu = model.readout[list(dataloaders["train"].keys())[0]].mu.squeeze()
                sigma = model.readout[list(dataloaders["train"].keys())[0]].sigma.squeeze()
                sample = model.readout[list(dataloaders["train"].keys())[0]].sample_grid(batch_size=1,
                                                                                         sample=True).squeeze()

                mu_x = torch.cat([mu_x, mu[:, 0][:, None]], dim=1)
                mu_y = torch.cat([mu_y, mu[:, 1][:, None]], dim=1)

                sigma_x = torch.cat([sigma_x, sigma[:, 0][:, None]], dim=1)
                sigma_y = torch.cat([sigma_y, sigma[:, 1][:, None]], dim=1)

                sample_x = torch.cat([sample_x, sample[:, 0][:, None]], dim=1)
                sample_y = torch.cat([sample_y, sample[:, 1][:, None]], dim=1)

                gauss_dict['epoch' + str(epoch)] = {'mu_x': mu_x.detach().cpu().numpy(),
                                                    'mu_y': mu_y.detach().cpu().numpy(),
                                                    'sigma_x': sigma_x.detach().cpu().numpy(),
                                                    'sigma_y': sigma_y.detach().cpu().numpy(),
